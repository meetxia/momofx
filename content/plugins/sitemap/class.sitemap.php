...
class sitemap {
    ...
    private function setData() {
        ...
        //日志
        $now = time();
        $query = $this->db->query("SELECT gid,date FROM " . DB_PREFIX . "blog WHERE type='blog' AND hide='n' AND date<= $now ORDER BY date DESC");
        ...
        while ($row = $this->db->fetch_array($query)) {
            $lastmod = $this->comment_time && isset($lastCommentTime[$row['gid']]) ? $lastCommentTime[$row['gid']] : $row['date'];
            $data[] = array('url' => Url::log($row['gid']), 'lastmod' => $lastmod, 'changefreq' => $this->changefreq[1], 'priority' => $this->priority[1]);
        }
        //分类
        ...
        foreach ($CACHE->readCache('record') as $value) {
            ...
            $data[] = array('url' => Url::record($value['date']), 'lastmod' => $lastmod, 'changefreq' => $this->changefreq[4], 'priority' => $this->priority[4]);
        }
        $this->data = $data;
    }
